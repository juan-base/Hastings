...
init = function() {
	...
	$(".portfolio-box").click(function(){
		if (currPortfolioFilter == "" || (currPortfolioFilter!="" && $(this).hasClass("portfolio-selected"))) {
			// TODO: Get info from here, load the portfolio and go there
			checkStartScrolling();
			//openPortfolio();
			addSinglePortfolio($(this).attr("data-objid"), true);
		} else {
			//console.log("Its not part of the results")
		}
	})
...
backFromSingle = function() {
	...
	setTimeout(function(){ 
		$("#single_portfolios .project-page").remove();
	}, milliseconds);
...
moveToPortfolio = function(changeRoute) {
...
appendSinglePortfolio = function(data, changeUrl, pos) {
	if ($("#page-"+data.id).length==0)	{
		var pos = typeof pos == "undefined" ? "after" : pos;
...
addSinglePortfolio = function(obj_id, changeUrl, scrollToIt) {
	//var changeUrl = !changeUrl || typeof changeUrl == "undefined" ? true : changeUrl;
	//var changeUrl = typeof changeUrl == "undefined" ? true : changeUrl;

	//console.log("addSinglePortfolio: " + obj_id, changeUrl);

	if (typeof historyRoutes.portfolio[obj_id] == "undefined")	{
		
		historyRoutes.portfolio[obj_id] = {};

		$.get(router_var.home.url + "/wp-content/themes/Hastings/get-page.php?id=" + obj_id, function(data) {
...
moveBackToPosition = function() {
	...
	var section = currPracticeFilter!=""?'practice':'portfolio'
		block = section=="practice"?"#two":"#three";
...
setPortfoliosRoute = function(url) {
	var routeObj = router_var.portfolios;

	if (router_var.portfolios.title == "" ) {
		$.get(router_var.home.url + "/wp-json/wp/v2/pages/" + router_var.portfolios.id, function(data) {
			router_var.portfolios.title = data.yoast_meta.yoast_wpseo_title;
			router_var.portfolios.url = data.link;
...
loadNextPortfolio = function(id) {
	var current = $(".portfolio-box[data-objid='"+id+"']"),
		selector = currPortfolioFilter == "" ? "" : ".portfolio-selected",
...
portfolio_scrolling = function() {

	$("#four").unbind("scroll").scroll(function() {
		$(".project-page").not(".next-loaded").each(function(){
			var $elem = $(this),
				visible = scrolledIntoView($elem, $(".project-page"));
		...
		$(".project-page").each(function(){
			...
			if ((elemTop >= 0 && elemTop < $(window).height() / 3) || (elemTop < 0 && elemTop + elemHeight > $(window).height())) {
				if (typeof historyRoutes.portfolio[id] != "undefined") {
					setRoute(historyRoutes.portfolio[id]);
				} else {
		...
		if (event.state.type == "home") {
			...
			if (viewingSinglePortfolio) {
				...
				if ($("#page-"+event.state.id).length == 0) {
					...
					addSinglePortfolio(event.state.id, false, true);
				} else {
					$("#page-"+event.state.id)[0].scrollIntoView();
				}
			} else {
				console.log("Load it and move to single portfolio");
				addSinglePortfolio(event.state.id, true);
