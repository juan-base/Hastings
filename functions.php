...
function identifyMobile() {
  ...
  <?php
  $output = ob_get_clean(); print $output;
  return true;
}
...
//    CUSTOM POST TYPES END====================================================
    ...
    function isMobile() {
      global $isMobile;
      $isMobile = (isset($_REQUEST['isMobile']) && (int)$_REQUEST['isMobile']==1) ? true : false;
      ...
      if (!$isMobile) {
          ...
          //$EXCLUDES = array('ipad');
          if (isset($INCLUDES)) foreach($INCLUDES as $I=>$device) if (strpos($HTTP_USER_AGENT,$device) !== FALSE) $isMobile = true;
          if (isset($EXCLUDES)) foreach($EXCLUDES as $I=>$device) if (strpos($HTTP_USER_AGENT,$device) !== FALSE) $isMobile = false;
      }
			return $isMobile;
    }
